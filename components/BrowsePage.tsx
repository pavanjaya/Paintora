...
export default function BrowsePage({
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (!(e.target as Element).closest('.browse-filter-dropdown-wrap')) {
      ...
      <main className="browse-main">
        ...
        <div ref={filterBarRef} className={`browse-filter-bar${sticky ? ' sticky' : ''}`}>
          <div className="browse-filter-inner">
            ...
            <div className="browse-filter-dropdown-wrap">
              <button
                className={`browse-filter-btn${filters.style ? ' has-value' : ''}${activeDropdown === 'style' ? ' active' : ''}`}
                onClick={() => setActiveDropdown(v => v === 'style' ? null : 'style')}
              >
              ...
              {activeDropdown === 'style' && (
                <div className="browse-dropdown">
                  {STYLE_OPTIONS.map(o => (
                    <button key={o} className={`browse-dropdown-item${filters.style === o ? ' selected' : ''}`} onClick={() => setFilter('style', o)}>{o}</button>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="browse-filter-dropdown-wrap">
              <button
                className={`browse-filter-btn${filters.medium ? ' has-value' : ''}${activeDropdown === 'medium' ? ' active' : ''}`}
                onClick={() => setActiveDropdown(v => v === 'medium' ? null : 'medium')}
              >
              ...
              {activeDropdown === 'medium' && (
                <div className="browse-dropdown">
                  {MEDIUM_OPTIONS.map(o => (
                    <button key={o} className={`browse-dropdown-item${filters.medium === o ? ' selected' : ''}`} onClick={() => setFilter('medium', o)}>{o}</button>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="browse-filter-dropdown-wrap">
              <button
                className={`browse-filter-btn${filters.subject ? ' has-value' : ''}${activeDropdown === 'subject' ? ' active' : ''}`}
                onClick={() => setActiveDropdown(v => v === 'subject' ? null : 'subject')}
              >
              ...
              {activeDropdown === 'subject' && (
                <div className="browse-dropdown">
                  {SUBJECT_OPTIONS.map(o => (
                    <button key={o} className={`browse-dropdown-item${filters.subject === o ? ' selected' : ''}`} onClick={() => setFilter('subject', o)}>{o}</button>
                  ))}
                </div>
              )}
            </div>
